...
func (h *Handler) ListLeads(c fiber.Ctx) error {
	...
	if err != nil {
		log.Error("leads: ListLeads", slog.Any("error", err))
...
func (h *Handler) GetLead(c fiber.Ctx) error {
	...
	if err != nil {
		...
		log.Error("leads: GetLead", slog.Any("error", err))
		return response.InternalServerError(c)
	}
...
func (h *Handler) CreateLead(c fiber.Ctx) error {
	...
	if err != nil {
		...
		log.Error("leads: CreateLead", slog.Any("error", err))
		return response.InternalServerError(c)
	}
...
func (h *Handler) UpdateLead(c fiber.Ctx) error {
	...
	if err != nil {
		...
		default:
			log.Error("leads: UpdateLead", slog.Any("error", err))
			return response.InternalServerError(c)
		}
	}
...
func (h *Handler) DeleteLead(c fiber.Ctx) error {
	...
	if err := h.service.DeleteLead(c.Context(), orgID(c), c.Params("leadId")); err != nil {
		...
		log.Error("leads: DeleteLead", slog.Any("error", err))
		return response.InternalServerError(c)
	}
...
func (h *Handler) ConvertLead(c fiber.Ctx) error {
	...
	if err != nil {
		...
		default:
			log.Error("leads: ConvertLead", slog.Any("error", err))
			return response.InternalServerError(c)
		}
	}
