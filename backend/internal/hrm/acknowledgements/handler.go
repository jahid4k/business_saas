...
func (h *Handler) List(c fiber.Ctx) error {
	...
	if err != nil {
		log.Error("acknowledgements: List", slog.Any("error", err))
...
func (h *Handler) ListByEntity(c fiber.Ctx) error {
	...
	if err != nil {
		log.Error("acknowledgements: ListByEntity", slog.Any("error", err))
		return response.InternalServerError(c)
	}
	return response.OK(c, res, "OK")
}
...
func (h *Handler) err(c fiber.Ctx, err error) error {
	...
	case errors.Is(err, ErrNotFound):
		return response.NotFound(c, "ACK_NOT_FOUND", "Acknowledgement not found")
	case errors.Is(err, ErrEmployeeIDRequired):
		return response.BadRequest(c, "EMPLOYEE_ID_REQUIRED", "employee_id is required")
	case errors.Is(err, ErrEntityTitleRequired):
		return response.BadRequest(c, "ENTITY_TITLE_REQUIRED", "entity_title is required")
	case errors.Is(err, ErrInvalidAckType):
		return response.BadRequest(c, "INVALID_ACK_TYPE", "acknowledgeable_type must be: warning, document, announcement, calendar_event, or policy")
	case errors.Is(err, ErrAckIDRequired):
		return response.BadRequest(c, "ACK_ID_REQUIRED", "acknowledgeable_id is required")
	case errors.Is(err, ErrInvalidDate):
		return response.BadRequest(c, "INVALID_DATE", "date must be a valid YYYY-MM-DD")
	case errors.Is(err, ErrWrongStatus):
		return response.Conflict(c, "WRONG_STATUS", "Action not allowed in current acknowledgement status")
	case errors.Is(err, ErrSignatureRequired):
		return response.BadRequest(c, "SIGNATURE_REQUIRED", "signature_data is required when signature_required=true")
	default:
		log.Error("acknowledgements: error", slog.Any("error", err))
		return response.InternalServerError(c)
	}
}
