...
func (h *Handler) ListRecords(c fiber.Ctx) error {
	...
	if err != nil {
		log.Error("attendance: ListRecords", slog.Any("error", err))
...
func (h *Handler) GetRecord(c fiber.Ctx) error {
	...
	scopeTier, err := h.authz.ResolveScope(c.Context(), userID, orgID, "hrm.attendance")
	if err != nil {
		log.Error("attendance: GetRecord", slog.Any("error", err))
		return response.InternalServerError(c)
	}
...
func (h *Handler) ListPeriods(c fiber.Ctx) error {
	...
	if err != nil {
		log.Error("attendance: ListPeriods", slog.Any("error", err))
		return response.InternalServerError(c)
	}
	return response.OK(c, res, "OK")
}
...
func (h *Handler) err(c fiber.Ctx, err error) error {
	...
	case errors.Is(err, ErrNotFound):
		return response.NotFound(c, "RECORD_NOT_FOUND", "Attendance record not found")
	case errors.Is(err, ErrPeriodNotFound):
		return response.NotFound(c, "PERIOD_NOT_FOUND", "Attendance period not found")
	...
	case errors.Is(err, ErrInvalidDayType):
		return response.BadRequest(c, "INVALID_DAY_TYPE", "invalid day_type value")
	case errors.Is(err, ErrDuplicateRecord):
		return response.Conflict(c, "DUPLICATE_RECORD", "Attendance record already exists for this employee on this date")
	case errors.Is(err, ErrPeriodFinalized):
		return response.Conflict(c, "PERIOD_FINALIZED", "Attendance period is finalized — no edits allowed")
	case errors.Is(err, ErrPeriodAlreadyFinalizedOrLocked):
		return response.Conflict(c, "PERIOD_ALREADY_FINALIZED", "Attendance period is already finalized or locked")
	case errors.Is(err, ErrPeriodNotOpen):
		return response.Conflict(c, "PERIOD_NOT_OPEN", "Attendance period must be open to finalize")
	...
	case errors.Is(err, ErrReasonRequired):
		return response.BadRequest(c, "REASON_REQUIRED", "Regularization reason is required")
	default:
		log.Error("attendance: error", slog.Any("error", err))
		return response.InternalServerError(c)
	}
}
