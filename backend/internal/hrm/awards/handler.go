...
func (h *Handler) List(c fiber.Ctx) error {
	...
	if err != nil {
		log.Error("awards: List", slog.Any("error", err))
...
func (h *Handler) err(c fiber.Ctx, err error) error {
	...
	case errors.Is(err, ErrNotFound):
		return response.NotFound(c, "AWARD_NOT_FOUND", "Award not found")
	...
	case errors.Is(err, ErrInvalidAwardType):
		return response.BadRequest(c, "INVALID_AWARD_TYPE", "invalid award_type")
	case errors.Is(err, ErrInvalidDate):
		return response.BadRequest(c, "INVALID_DATE", "date must be a valid YYYY-MM-DD")
	case errors.Is(err, ErrWrongStatus):
		return response.Conflict(c, "WRONG_STATUS", "Action not allowed in current award status")
	case errors.Is(err, ErrAlreadyIssued):
		return response.Conflict(c, "ALREADY_ISSUED", "Award has already been issued")
	default:
		log.Error("awards: error", slog.Any("error", err))
		return response.InternalServerError(c)
	}
}
