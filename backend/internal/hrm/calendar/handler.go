...
func (h *Handler) List(c fiber.Ctx) error {
	...
	if err != nil {
		log.Error("calendar: List", slog.Any("error", err))
...
func (h *Handler) err(c fiber.Ctx, err error) error {
	...
	case errors.Is(err, ErrNotFound):
		return response.NotFound(c, "EVENT_NOT_FOUND", "Calendar event not found")
	case errors.Is(err, ErrTitleRequired):
		return response.BadRequest(c, "TITLE_REQUIRED", "title is required")
	case errors.Is(err, ErrInvalidEventType):
		return response.BadRequest(c, "INVALID_EVENT_TYPE", "invalid event_type")
	...
	case errors.Is(err, ErrEndDateRequired):
		return response.BadRequest(c, "END_DATE_REQUIRED", "end_date is required (YYYY-MM-DD)")
	case errors.Is(err, ErrInvalidDate):
		return response.BadRequest(c, "INVALID_DATE", "date must be a valid YYYY-MM-DD")
	case errors.Is(err, ErrEndBeforeStart):
		return response.BadRequest(c, "END_BEFORE_START", "end_date must be >= start_date")
	case errors.Is(err, ErrWrongStatus):
		return response.Conflict(c, "WRONG_STATUS", "Action not allowed in current event status")
	default:
		log.Error("calendar: error", slog.Any("error", err))
		return response.InternalServerError(c)
	}
}
