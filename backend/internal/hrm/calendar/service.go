...
func (s *serviceImpl) List(ctx context.Context, orgID, eventType, status, fromDate, toDate string) (*EventListResponse, error) {
	...
	if err != nil { return nil, fmt.Errorf("calendar: List: %w", err) }
	if list == nil { list = []*CalendarEvent{} }
...
func (s *serviceImpl) Get(ctx context.Context, orgID, ref string) (*CalendarEvent, error) {
	...
	if err != nil { return nil, fmt.Errorf("calendar: Get: %w", err) }
...
func (s *serviceImpl) Create(ctx context.Context, orgID, createdBy string, req CreateEventRequest) (*CalendarEvent, error) {
	...
	if !req.EventType.IsValid() { return nil, ErrInvalidEventType }
	if strings.TrimSpace(req.StartDate) == "" { return nil, ErrStartDateRequired }
	if strings.TrimSpace(req.EndDate) == "" { return nil, ErrEndDateRequired }
	s1, e1 := time.Parse(dateLayout, req.StartDate), error(nil)
	if _, err := time.Parse(dateLayout, req.StartDate); err != nil { e1 = err }
	if _, err := time.Parse(dateLayout, req.EndDate); err != nil { return nil, ErrInvalidDate }
	if e1 != nil { return nil, ErrInvalidDate }
	if req.EndDate < req.StartDate { return nil, ErrEndBeforeStart }
	_ = s1
	scope := req.ScopeType
	...
	if err := s.repo.Create(ctx, e); err != nil { return nil, fmt.Errorf("calendar: Create: %w", err) }
...
func (s *serviceImpl) Update(ctx context.Context, orgID, ref string, req UpdateEventRequest) (*CalendarEvent, error) {
	e, err := s.repo.FindByRef(ctx, orgID, ref)
	if err != nil { return nil, fmt.Errorf("calendar: Update: %w", err) }
	if e == nil { return nil, ErrNotFound }
	if e.Status == StatusCancelled { return nil, ErrWrongStatus }
	if req.Title != nil { e.Title = *req.Title }
	if req.Description != nil { e.Description = req.Description }
	if req.StartDate != nil {
		if _, err := time.Parse(dateLayout, *req.StartDate); err != nil { return nil, ErrInvalidDate }
	...
	if req.Location != nil { e.Location = req.Location }
	if req.RequiresRSVP != nil { e.RequiresRSVP = *req.RequiresRSVP }
	if req.RSVPDeadline != nil { e.RSVPDeadline = req.RSVPDeadline }
	if req.Status != nil { e.Status = *req.Status }
	if err := s.repo.Update(ctx, e); err != nil { return nil, fmt.Errorf("calendar: Update: %w", err) }
	return e, nil
}

func (s *serviceImpl) Cancel(ctx context.Context, orgID, ref string) (*CalendarEvent, error) {
	e, err := s.repo.FindByRef(ctx, orgID, ref)
	if err != nil { return nil, fmt.Errorf("calendar: Cancel: %w", err) }
	if e == nil { return nil, ErrNotFound }
	if e.Status == StatusCancelled { return nil, ErrWrongStatus }
	if err := s.repo.UpdateStatus(ctx, e.ID, StatusCancelled); err != nil { return nil, fmt.Errorf("calendar: Cancel: %w", err) }
...
func (s *serviceImpl) RequestRSVP(ctx context.Context, orgID, ref, requestedBy string) (*CalendarEvent, error) {
	e, err := s.repo.FindByRef(ctx, orgID, ref)
	if err != nil { return nil, fmt.Errorf("calendar: RequestRSVP: %w", err) }
	if e == nil { return nil, ErrNotFound }
...
func (s *serviceImpl) requestRSVPInternal(ctx context.Context, orgID string, e *CalendarEvent, requestedBy string) (*CalendarEvent, error) {
	...
	if err != nil { return e, nil } // non-fatal
