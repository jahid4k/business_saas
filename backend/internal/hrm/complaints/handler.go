...
func (h *Handler) ListAll(c fiber.Ctx) error {
	...
	if err != nil {
		log.Error("complaints: ListAll", slog.Any("error", err))
...
func (h *Handler) ListForEmployee(c fiber.Ctx) error {
	...
	filter, err := h.resolveListFilter(c, orgID, userID)
	if err != nil {
		log.Error("complaints: ListForEmployee", slog.Any("error", err))
		return response.InternalServerError(c)
	}
...
func (h *Handler) Get(c fiber.Ctx) error {
	...
	scopeTier, err := h.authz.ResolveScope(c.Context(), userID, orgID, "hrm.complaints")
	if err != nil {
		log.Error("complaints: Get", slog.Any("error", err))
		return response.InternalServerError(c)
	}
...
func (h *Handler) err(c fiber.Ctx, err error) error {
	...
	case errors.Is(err, ErrNotFound):
		return response.NotFound(c, "COMPLAINT_NOT_FOUND", "Complaint not found")
	...
	case errors.Is(err, ErrInvalidType):
		return response.BadRequest(c, "INVALID_TYPE", "Invalid complaint_type")
	case errors.Is(err, ErrInvalidDate):
		return response.BadRequest(c, "INVALID_DATE", "date must be a valid YYYY-MM-DD")
	case errors.Is(err, ErrWrongStatus):
		return response.Conflict(c, "WRONG_STATUS", "Action not allowed in current complaint status")
	case errors.Is(err, ErrInvestigatorRequired):
		return response.BadRequest(c, "INVESTIGATOR_REQUIRED", "investigator_id is required")
	case errors.Is(err, ErrResolutionRequired):
		return response.BadRequest(c, "RESOLUTION_REQUIRED", "resolution text is required")
	default:
		log.Error("complaints: error", slog.Any("error", err))
		return response.InternalServerError(c)
	}
}
