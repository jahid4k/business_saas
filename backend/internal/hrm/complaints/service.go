...
func (s *serviceImpl) Get(ctx context.Context, orgID, employeeID, ref string) (*Complaint, error) {
	...
	if err != nil { return nil, fmt.Errorf("complaints: Get: %w", err) }
	if c == nil { return nil, ErrNotFound }
	return c, nil
}
...
func (s *serviceImpl) Create(ctx context.Context, orgID, employeeID, createdBy string, req CreateComplaintRequest) (*Complaint, error) {
	...
	if !req.ComplaintType.IsValid() { req.ComplaintType = TypeGeneral }
	...
	if err := s.repo.Create(ctx, c); err != nil { return nil, fmt.Errorf("complaints: Create: %w", err) }
	return c, nil
}

func (s *serviceImpl) Update(ctx context.Context, orgID, employeeID, ref string, req UpdateComplaintRequest) (*Complaint, error) {
	c, err := s.repo.FindByRef(ctx, orgID, employeeID, ref)
	if err != nil { return nil, fmt.Errorf("complaints: Update: %w", err) }
	if c == nil { return nil, ErrNotFound }
	if c.Status == StatusResolved || c.Status == StatusDismissed || c.Status == StatusWithdrawn { return nil, ErrWrongStatus }
	...
	if req.AgainstDetails != nil { c.AgainstDetails = req.AgainstDetails }
	if req.DocumentID != nil { c.DocumentID = req.DocumentID }
	if err := s.repo.Update(ctx, c); err != nil { return nil, fmt.Errorf("complaints: Update: %w", err) }
	return c, nil
}

func (s *serviceImpl) StartReview(ctx context.Context, orgID, employeeID, ref string) (*Complaint, error) {
	c, err := s.repo.FindByRef(ctx, orgID, employeeID, ref)
	if err != nil { return nil, fmt.Errorf("complaints: StartReview: %w", err) }
	if c == nil { return nil, ErrNotFound }
	if c.Status != StatusSubmitted { return nil, ErrWrongStatus }
	if err := s.repo.UpdateStatus(ctx, c.ID, StatusUnderReview); err != nil { return nil, fmt.Errorf("complaints: StartReview: %w", err) }
...
func (s *serviceImpl) Assign(ctx context.Context, orgID, employeeID, ref string, req AssignRequest) (*Complaint, error) {
	if strings.TrimSpace(req.InvestigatorID) == "" { return nil, ErrInvestigatorRequired }
	c, err := s.repo.FindByRef(ctx, orgID, employeeID, ref)
	if err != nil { return nil, fmt.Errorf("complaints: Assign: %w", err) }
	if c == nil { return nil, ErrNotFound }
	if c.Status == StatusResolved || c.Status == StatusDismissed || c.Status == StatusWithdrawn { return nil, ErrWrongStatus }
	now := time.Now()
	...
	if err := s.repo.Update(ctx, c); err != nil { return nil, fmt.Errorf("complaints: Assign: update: %w", err) }
	if err := s.repo.UpdateStatus(ctx, c.ID, StatusInvestigating); err != nil { return nil, fmt.Errorf("complaints: Assign: status: %w", err) }
	return c, nil
}

func (s *serviceImpl) Resolve(ctx context.Context, orgID, employeeID, ref, resolvedBy string, req ResolveRequest) (*Complaint, error) {
	if strings.TrimSpace(req.Resolution) == "" { return nil, ErrResolutionRequired }
	c, err := s.repo.FindByRef(ctx, orgID, employeeID, ref)
	if err != nil { return nil, fmt.Errorf("complaints: Resolve: %w", err) }
	if c == nil { return nil, ErrNotFound }
	if c.Status == StatusResolved || c.Status == StatusDismissed || c.Status == StatusWithdrawn { return nil, ErrWrongStatus }
	now := time.Now()
	...
	if err := s.repo.Update(ctx, c); err != nil { return nil, fmt.Errorf("complaints: Resolve: update: %w", err) }
	if err := s.repo.UpdateStatus(ctx, c.ID, StatusResolved); err != nil { return nil, fmt.Errorf("complaints: Resolve: status: %w", err) }
	return c, nil
}
...
func (s *serviceImpl) Dismiss(ctx context.Context, orgID, employeeID, ref, resolvedBy string, req DismissRequest) (*Complaint, error) {
	if strings.TrimSpace(req.Resolution) == "" { return nil, ErrResolutionRequired }
	c, err := s.repo.FindByRef(ctx, orgID, employeeID, ref)
	if err != nil { return nil, fmt.Errorf("complaints: Dismiss: %w", err) }
	if c == nil { return nil, ErrNotFound }
	if c.Status == StatusResolved || c.Status == StatusDismissed || c.Status == StatusWithdrawn { return nil, ErrWrongStatus }
	now := time.Now()
	c.Resolution = &req.Resolution
	...
	if err := s.repo.Update(ctx, c); err != nil { return nil, fmt.Errorf("complaints: Dismiss: update: %w", err) }
	if err := s.repo.UpdateStatus(ctx, c.ID, StatusDismissed); err != nil { return nil, fmt.Errorf("complaints: Dismiss: status: %w", err) }
	return c, nil
}

func (s *serviceImpl) Withdraw(ctx context.Context, orgID, employeeID, ref string) (*Complaint, error) {
	c, err := s.repo.FindByRef(ctx, orgID, employeeID, ref)
	if err != nil { return nil, fmt.Errorf("complaints: Withdraw: %w", err) }
	if c == nil { return nil, ErrNotFound }
	if c.Status == StatusResolved || c.Status == StatusDismissed { return nil, ErrWrongStatus }
	if err := s.repo.UpdateStatus(ctx, c.ID, StatusWithdrawn); err != nil { return nil, fmt.Errorf("complaints: Withdraw: %w", err) }
