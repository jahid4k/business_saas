...
func (s *serviceImpl) GetActive(ctx context.Context, orgID, employeeID string) (*EmployeeContract, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("contracts: GetActive: %w", err)
	}
	if c == nil {
...
func (s *serviceImpl) Create(ctx context.Context, orgID, employeeID, createdBy string, req CreateContractRequest) (*EmployeeContract, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("contracts: Create: check active: %w", err)
	}
	if existing != nil {
		return nil, ErrActiveContractExists
	}
...
func (s *serviceImpl) Update(ctx context.Context, orgID, employeeID, ref string, req UpdateContractRequest) (*EmployeeContract, error) {
	c, err := s.repo.FindByRef(ctx, orgID, employeeID, ref)
	if err != nil {
		return nil, fmt.Errorf("contracts: Update: %w", err)
	}
	if c == nil {
		return nil, ErrContractNotFound
	}
	...
	if req.ProbationEndDate != nil {
		c.ProbationEndDate = req.ProbationEndDate
	}
	if req.NoticePeriodDays != nil {
		c.NoticePeriodDays = *req.NoticePeriodDays
	}
	if req.SalaryStructureID != nil {
		c.SalaryStructureID = req.SalaryStructureID
	}
	if req.WorkHoursPerWeek != nil {
		c.WorkHoursPerWeek = req.WorkHoursPerWeek
	}
