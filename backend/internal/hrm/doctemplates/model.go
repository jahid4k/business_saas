...
const (
	...
	// DocTypeRelievingLetter backs Phase 9C. ⚠ Widening the enum WITHOUT the
	// matching IsValid() case is exactly how AckType went two phases with two
	// DB-legal values unreachable through the only typed write path (see
	// r29) — both are changed together here, and both document_type CHECKs
	// were widened in migration 00118.
	DocTypeRelievingLetter DocumentType = "relieving_letter"
...
func (d DocumentType) IsValid() bool {
	...
	case DocTypeOfferLetter, DocTypeContract, DocTypeWarningLetter, DocTypePromotionLetter,
		...
		DocTypeExperienceLetter, DocTypeRelievingLetter, DocTypeNDA, DocTypePolicy, DocTypeCustom:
		return true
	}
	return false
}
