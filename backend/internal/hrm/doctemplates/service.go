...
func (s *serviceImpl) List(ctx context.Context, orgID string, activeOnly bool, docType string) (*DocumentTemplateListResponse, error) {
	...
	if err != nil { return nil, fmt.Errorf("doctemplates: List: %w", err) }
	if list == nil { list = []*DocumentTemplate{} }
...
func (s *serviceImpl) Get(ctx context.Context, orgID, ref string) (*DocumentTemplate, error) {
	...
	if err != nil { return nil, fmt.Errorf("doctemplates: Get: %w", err) }
...
func (s *serviceImpl) Create(ctx context.Context, orgID, createdBy string, req CreateDocumentTemplateRequest) (*DocumentTemplate, error) {
	...
	if !req.DocumentType.IsValid() { return nil, ErrInvalidDocumentType }
	if strings.TrimSpace(req.BodyMarkdown) == "" { return nil, ErrBodyRequired }
	if exists, _ := s.repo.NameExists(ctx, orgID, name, ""); exists { return nil, ErrNameConflict }

	vars := req.AvailableVariables
	if len(vars) == 0 { vars = []string{} }
	...
	if err := s.repo.Create(ctx, t); err != nil { return nil, fmt.Errorf("doctemplates: Create: %w", err) }
	return t, nil
}

func (s *serviceImpl) Update(ctx context.Context, orgID, ref string, req UpdateDocumentTemplateRequest) (*DocumentTemplate, error) {
	t, err := s.repo.FindByRef(ctx, orgID, ref)
	if err != nil { return nil, fmt.Errorf("doctemplates: Update: %w", err) }
	if t == nil { return nil, ErrTemplateNotFound }
	...
	if req.Name != nil {
		...
		if name == "" { return nil, ErrNameRequired }
		if len(name) > 100 { return nil, ErrNameTooLong }
		if exists, _ := s.repo.NameExists(ctx, orgID, name, t.ID); exists { return nil, ErrNameConflict }
		t.Name = name
	}
	...
	if req.AvailableVariables != nil { t.AvailableVariables = req.AvailableVariables }
	if req.RequiresAcknowledgement != nil { t.RequiresAcknowledgement = *req.RequiresAcknowledgement }
	if req.IsActive != nil { t.IsActive = *req.IsActive }

	if err := s.repo.Update(ctx, t); err != nil { return nil, fmt.Errorf("doctemplates: Update: %w", err) }
	return t, nil
}
...
func (s *serviceImpl) Preview(ctx context.Context, orgID, ref string, req PreviewTemplateRequest) (*PreviewResult, error) {
	t, err := s.repo.FindByRef(ctx, orgID, ref)
	if err != nil { return nil, fmt.Errorf("doctemplates: Preview: %w", err) }
	if t == nil { return nil, ErrTemplateNotFound }
