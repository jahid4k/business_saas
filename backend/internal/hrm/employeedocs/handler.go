...
func (h *Handler) List(c fiber.Ctx) error {
	...
	if err != nil {
		log.Error("employeedocs: List", slog.Any("error", err))
...
func (h *Handler) ListAll(c fiber.Ctx) error {
	...
	filter, err := h.resolveListFilter(c, orgID, userID)
	if err != nil {
		log.Error("employeedocs: ListAll", slog.Any("error", err))
		return response.InternalServerError(c)
	}
...
func (h *Handler) Get(c fiber.Ctx) error {
	...
	scopeTier, err := h.authz.ResolveScope(c.Context(), userID, orgID, "hrm.documents")
	if err != nil {
		log.Error("employeedocs: Get", slog.Any("error", err))
		return response.InternalServerError(c)
	}
...
func (h *Handler) err(c fiber.Ctx, err error) error {
	...
	case errors.Is(err, ErrNotFound):
		return response.NotFound(c, "DOCUMENT_NOT_FOUND", "Document not found")
	case errors.Is(err, ErrTitleRequired):
		return response.BadRequest(c, "TITLE_REQUIRED", "title is required")
	case errors.Is(err, ErrFileURLRequired):
		return response.BadRequest(c, "FILE_URL_REQUIRED", "file_url is required")
	case errors.Is(err, ErrFileNameRequired):
		return response.BadRequest(c, "FILE_NAME_REQUIRED", "file_name is required")
	case errors.Is(err, ErrDocTypeRequired):
		return response.BadRequest(c, "DOC_TYPE_REQUIRED", "document_type is required")
	case errors.Is(err, ErrWrongStatus):
		return response.Conflict(c, "WRONG_STATUS", "Action not allowed in current document status")
	case errors.Is(err, ErrAlreadyAcknowledged):
		return response.Conflict(c, "ALREADY_ACKNOWLEDGED", "Document has already been acknowledged")
	default:
		log.Error("employeedocs: error", slog.Any("error", err))
		return response.InternalServerError(c)
	}
}
