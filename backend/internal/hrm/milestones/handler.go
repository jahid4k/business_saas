...
func (h *Handler) List(c fiber.Ctx) error {
	...
	if err != nil {
		log.Error("milestones: List", slog.Any("error", err))
...
func (h *Handler) Generate(c fiber.Ctx) error {
	...
	if err != nil {
		return response.BadRequest(c, "GENERATE_FAILED", err.Error())
	}
...
func (h *Handler) err(c fiber.Ctx, err error) error {
	...
	case errors.Is(err, ErrNotFound):
		return response.NotFound(c, "MILESTONE_NOT_FOUND", "Milestone not found")
	...
	case errors.Is(err, ErrInvalidMilestoneType):
		return response.BadRequest(c, "INVALID_MILESTONE_TYPE", "invalid milestone_type")
	case errors.Is(err, ErrDateRequired):
		return response.BadRequest(c, "DATE_REQUIRED", "milestone_date is required (YYYY-MM-DD)")
	case errors.Is(err, ErrInvalidDate):
		return response.BadRequest(c, "INVALID_DATE", "date must be a valid YYYY-MM-DD")
	case errors.Is(err, ErrAlreadyAcknowledged):
		return response.Conflict(c, "ALREADY_ACKNOWLEDGED", "Milestone has already been acknowledged")
	case errors.Is(err, ErrDuplicate):
		return response.Conflict(c, "DUPLICATE", "Milestone already exists for this employee, type, and date")
	default:
		log.Error("milestones: error", slog.Any("error", err))
		return response.InternalServerError(c)
	}
}
