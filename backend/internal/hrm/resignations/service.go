...
type Service interface {
	List(ctx context.Context, orgID, employeeID, status string) (*ResignationListResponse, error)
...
func (s *serviceImpl) List(ctx context.Context, orgID, employeeID, status string) (*ResignationListResponse, error) {
	list, err := s.repo.FindAll(ctx, orgID, employeeID, status)
	if err != nil { return nil, fmt.Errorf("resignations: List: %w", err) }
	if list == nil { list = []*Resignation{} }
	return &ResignationListResponse{Resignations: list, Total: len(list)}, nil
}
...
func (s *serviceImpl) Get(ctx context.Context, orgID, employeeID, ref string) (*Resignation, error) {
	...
	if err != nil { return nil, fmt.Errorf("resignations: Get: %w", err) }
...
func (s *serviceImpl) Submit(ctx context.Context, orgID, employeeID, createdBy string, req SubmitResignationRequest) (*Resignation, error) {
	if strings.TrimSpace(req.ResignationDate) == "" { return nil, ErrResignationDateReq }
	resDate, err := time.Parse(dateLayout, req.ResignationDate)
	if err != nil { return nil, ErrInvalidDate }
	if !req.ReasonCategory.IsValid() { req.ReasonCategory = ReasonOther }

	// Only one active resignation per employee
	existing, err := s.repo.FindActiveByEmployee(ctx, orgID, employeeID)
	if err != nil { return nil, fmt.Errorf("resignations: Submit: check active: %w", err) }
	if existing != nil { return nil, ErrAlreadyActive }
	...
	if contractNoticeDays != nil { noticeDays = *contractNoticeDays }
	...
	if err := s.repo.Create(ctx, r); err != nil { return nil, fmt.Errorf("resignations: Submit: %w", err) }
	return r, nil
}

func (s *serviceImpl) Update(ctx context.Context, orgID, employeeID, ref string, req UpdateResignationRequest) (*Resignation, error) {
	r, err := s.repo.FindByRef(ctx, orgID, employeeID, ref)
	if err != nil { return nil, fmt.Errorf("resignations: Update: %w", err) }
	if r == nil { return nil, ErrNotFound }
	if r.Status == StatusWithdrawn || r.Status == StatusRejected { return nil, ErrWrongStatus }
	if req.ExitInterviewCompleted != nil { r.ExitInterviewCompleted = *req.ExitInterviewCompleted }
	if req.ExitClearanceCompleted != nil { r.ExitClearanceCompleted = *req.ExitClearanceCompleted }
	if req.DocumentID != nil { r.DocumentID = req.DocumentID }
	if req.LastWorkingDate != nil {
		if _, err := time.Parse(dateLayout, *req.LastWorkingDate); err != nil { return nil, ErrInvalidDate }
		r.LastWorkingDate = *req.LastWorkingDate
	}
	if err := s.repo.Update(ctx, r); err != nil { return nil, fmt.Errorf("resignations: Update: %w", err) }
	return r, nil
}

func (s *serviceImpl) Withdraw(ctx context.Context, orgID, employeeID, ref string) (*Resignation, error) {
	r, err := s.repo.FindByRef(ctx, orgID, employeeID, ref)
	if err != nil { return nil, fmt.Errorf("resignations: Withdraw: %w", err) }
	if r == nil { return nil, ErrNotFound }
	if r.Status != StatusSubmitted { return nil, ErrWrongStatus }
...
func (s *serviceImpl) Accept(ctx context.Context, orgID, employeeID, ref, acceptedBy string) (*Resignation, error) {
	r, err := s.repo.FindByRef(ctx, orgID, employeeID, ref)
	if err != nil { return nil, fmt.Errorf("resignations: Accept: %w", err) }
	if r == nil { return nil, ErrNotFound }
	if r.Status == StatusAccepted { return nil, ErrAlreadyAccepted }
	if r.Status != StatusSubmitted { return nil, ErrWrongStatus }

	tx, err := s.db.Begin(ctx)
	if err != nil { return nil, fmt.Errorf("resignations: Accept: begin tx: %w", err) }
	...
	// Set employee status to 'resigned' and record the last_working_date as termination_date.
	// Employee remains payroll-active until last_working_date.
	// The 'resigned' status requires migration 00031 which extended the status CHECK constraint.
	if _, err = tx.Exec(ctx,
		`UPDATE hrm_employees SET status='resigned', termination_date=$1::date, updated_at=NOW()
		WHERE id=$2::uuid AND org_id=$3::uuid`,
		r.LastWorkingDate, r.EmployeeID, r.OrgID,
	); err != nil {
		return nil, fmt.Errorf("resignations: Accept: update employee: %w", err)
	}

	if err := tx.Commit(ctx); err != nil { return nil, fmt.Errorf("resignations: Accept: commit: %w", err) }
...
func (s *serviceImpl) Reject(ctx context.Context, orgID, employeeID, ref string) (*Resignation, error) {
	r, err := s.repo.FindByRef(ctx, orgID, employeeID, ref)
	if err != nil { return nil, fmt.Errorf("resignations: Reject: %w", err) }
	if r == nil { return nil, ErrNotFound }
	if r.Status != StatusSubmitted { return nil, ErrWrongStatus }
