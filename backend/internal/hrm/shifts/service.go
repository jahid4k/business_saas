...
func (s *serviceImpl) List(ctx context.Context, orgID string, activeOnly bool) (*ShiftListResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("shifts: List: %w", err)
	}
	if list == nil {
		list = []*Shift{}
	}
...
func (s *serviceImpl) Get(ctx context.Context, orgID, ref string) (*Shift, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("shifts: Get: %w", err)
	}
	if sh == nil {
...
func (s *serviceImpl) Create(ctx context.Context, orgID, createdBy string, req CreateShiftRequest) (*Shift, error) {
	...
	if !req.ShiftType.IsValid() {
		return nil, ErrInvalidShiftType
	}
	if req.ShiftType == ShiftTypeFixed && (req.StartTime == nil || req.EndTime == nil) {
		return nil, ErrFixedTimeRequired
	}
	if req.ShiftType == ShiftTypeFlexible && req.WeeklyHoursTarget == nil {
		return nil, ErrFlexHoursRequired
	}
	...
	if err := s.repo.Create(ctx, sh); err != nil {
		return nil, fmt.Errorf("shifts: Create: %w", err)
	}
	return sh, nil
}
...
func (s *serviceImpl) Update(ctx context.Context, orgID, ref string, req UpdateShiftRequest) (*Shift, error) {
	sh, err := s.repo.FindByRef(ctx, orgID, ref)
	if err != nil {
		return nil, fmt.Errorf("shifts: Update: %w", err)
	}
	if sh == nil {
		return nil, ErrShiftNotFound
	}
	...
	if req.Name != nil {
		...
		if exists, _ := s.repo.NameExists(ctx, orgID, n, sh.ID); exists {
			return nil, ErrNameConflict
		}
		sh.Name = n
	}
	if req.Description != nil {
		sh.Description = req.Description
	}
	...
	if req.CoreStartTime != nil {
		sh.CoreStartTime = req.CoreStartTime
	}
	if req.CoreEndTime != nil {
		sh.CoreEndTime = req.CoreEndTime
	}
	if req.WeeklyHoursTarget != nil {
		sh.WeeklyHoursTarget = req.WeeklyHoursTarget
	}
	if req.BreakMinutes != nil {
		sh.BreakMinutes = *req.BreakMinutes
	}
	if req.WorkingDays != nil {
		sh.WorkingDays = req.WorkingDays
	}
	if req.TrackOvertime != nil {
		sh.TrackOvertime = *req.TrackOvertime
	}
	if req.OvertimeThresholdHours != nil {
		sh.OvertimeThresholdHours = req.OvertimeThresholdHours
	}
	if req.TrackBreaks != nil {
		sh.TrackBreaks = *req.TrackBreaks
	}
	if req.IsDefault != nil {
		sh.IsDefault = *req.IsDefault
	}
	if req.IsActive != nil {
		sh.IsActive = *req.IsActive
	}
...
func (s *serviceImpl) ListAssignments(ctx context.Context, orgID, assigneeType, assigneeID string) (*AssignmentListResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("shifts: ListAssignments: %w", err)
	}
	if list == nil {
		list = []*WorkScheduleAssignment{}
	}
...
func (s *serviceImpl) Assign(ctx context.Context, orgID, createdBy string, req AssignShiftRequest) (*WorkScheduleAssignment, error) {
	...
	if err := s.repo.CreateAssignment(ctx, a); err != nil {
		return nil, fmt.Errorf("shifts: Assign: %w", err)
	}
	return a, nil
}
