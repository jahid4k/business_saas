          ...
          {listQuery.isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center p-16 text-center bg-(--bg-surface) rounded-xl border border-(--border) border-dashed">
              ...
              {canManage && (
                <button
                  onClick={openCreate}
                  className="inline-flex items-center gap-2 rounded-lg bg-(--bg-surface) border border-(--border) px-4 py-2 text-sm font-medium text-(--text-primary) shadow-sm hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
                >
                  <Plus size={16} />
                  Add First Template
                </button>
            ...
            <div className="bg-(--bg-surface) border border-(--border) rounded-xl overflow-hidden shadow-sm">
              <div className="grid grid-cols-12 gap-4 border-b border-(--border) bg-gray-50/50 dark:bg-white/2 p-4 text-xs font-semibold text-(--text-secondary) uppercase tracking-wider">
              ...
              <div className="divide-y divide-(--border)">
                {items.map((template) => (
                  <div key={template.id} className="grid grid-cols-12 gap-4 items-center p-4 hover:bg-gray-50/50 dark:hover:bg-white/2 transition-colors">
                      ...
                      {canManage ? (
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="p-1.5 text-(--text-muted) hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-500/10 rounded transition-colors"
                            title="Edit"
                          >
                            <Pencil size={16} />
                          </button>
                          ...
                          {deleteConfirm === template.id ? (
                            <div className="flex items-center gap-1 animate-in fade-in zoom-in duration-200">
                              ...
                              <button
                                ...
                                className="px-2 py-1 bg-(--bg-surface) hover:bg-gray-100 dark:hover:bg-white/5 border border-(--border) text-(--text-secondary) text-xs font-medium rounded transition-colors"
                              >
                                Cancel
                              </button>
                            </div>
                            ...
                            <button
                              ...
                              className="p-1.5 text-(--text-muted) hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 rounded transition-colors"
                              title="Delete"
                            >
                              <Trash2 size={16} />
                            </button>
